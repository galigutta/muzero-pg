    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                print(
                    f'Last test reward: {info["total_reward"]:.2f}. Training step: {info["training_step"]}/{self.config.training_steps}. Played games: {info["num_played_games"]}. Loss: {info["total_loss"]:.2f}. Reanalyzed: {info["num_reanalysed_games"]}.   ',
                    end="\r",
                )
                counter += 1
                time.sleep(0.5)
